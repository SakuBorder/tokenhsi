...
#             self.gym.subscribe_viewer_keyboard_event(
#                 self.viewer, gymapi.KEY_PERIOD, "single_step")   # 监听"." 键
#             self.gym.subscribe_viewer_keyboard_event(
#                 self.viewer, gymapi.KEY_C, "toggle_contacts")   # ← C 键开关
#             self._contacts_visible = False     # 当前是否显示箭头
#             self._contact_thres   = 1.0        # 只显示 >1 N 的接触
#             self._contact_scale   = 0.01       # 箭头长度缩放
...
#                 elif evt.action == "single_step" and evt.value > 0:
#                     # 按 "." 时，仅在下一次 _physics_step() 前进一帧
...
#             else:
#                 self.gym.step_graphics(self.sim)
#                 self.gym.draw_viewer(self.viewer, self.sim, True)   # ← 关键
#                 self.gym.poll_viewer_events(self.viewer)
#                 # self.gym.poll_viewer_events(self.viewer)
...
#             if self.enable_viewer_sync or getattr(self, '_do_single_step', False):
#                 self.gym.simulate(self.sim)   # 正常前进
#                 self._do_single_step = False  # 清一次标记
#         return
